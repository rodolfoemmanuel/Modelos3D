...
function query(fn, keys2 = null, preload2 = false, config = {}) {
  ...
  const entry = {
    ...
    promise: (isPromise(fn) ? fn : fn(...keys2)).then((response) => {
...
function applyProps$1(instance, data) {
  ...
  for (let i2 = 0; i2 < changes.length; i2++) {
    ...
    if (isEvent) {
      ...
      if (currentInstance[key] instanceof Texture && currentInstance[key].format === RGBAFormat && currentInstance[key].type === UnsignedByteType) {
...
function Portal({
  ...
  const inject = React.useCallback((rootState, injectState) => {
    ...
    Object.keys(rootState).forEach((key) => {
      if (privateKeys.includes(key) || rootState[key] !== injectState[key] && injectState[key]) {
 ...
 * LICENSE file in the root directory of this source tree.
 */
//# sourceMappingURL=chunk-C7RNY4MB.js.map
